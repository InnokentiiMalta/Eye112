...
2. **Загрузите модель (два способа):**
   
   **Способ 1: Выбор файла (рекомендуется для .exe)**
   - Нажмите "Выбрать файл модели"
   - Выберите .onnx файл с компьютера
   - Дождитесь сообщения "✓ Модель загружена"
   
   **Способ 2: Загрузка по URL**
   - Введите URL модели (по умолчанию: GitHub)
   - Нажмите "Загрузить по URL"
   - Дождитесь сообщения "✓ Модель загружена"
